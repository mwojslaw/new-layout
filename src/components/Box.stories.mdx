...
-   Primitive component for all your layout needs
-   Ergonomic and responsive values
-   Themeable
...
When building business components it could be a good idea, not to expose complete length api.
Maybe we want just space api for consistency.
...
export const Text: React.FC<TextProps> = ({ children, ...props }) => <Box display="inline-block" {...props}>{children}</Box>;
```
